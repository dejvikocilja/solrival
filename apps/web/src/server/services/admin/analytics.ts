 ...
 *
 * Literal `Prisma.sql` fragments from a closed set — nothing derived from user
 * input is interpolated into SQL text, so injection is structurally impossible.
 *
 * They MUST be embedded via the FUNCTION form `prisma.$queryRaw(Prisma.sql`…`)`.
 * In the tagged-template form every `${}` is bound as a query parameter,
 * including SQL fragments: Prisma serialises the fragment to JSON and Postgres
 * then rejects `date_trunc(jsonb, timestamptz)`.
 */
